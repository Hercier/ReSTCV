...
def render_graph_ReSTIRPT():
    ...
    ReSTIRGIPlusPass = createPass("ReSTIRPTPass", {'samplesPerPixel': 1,'CVMode':ReSTIRCVMode.ReSTCV})#,'shiftStrategy': ShiftMapping.RandomReplay,'CVMode':ReSTIRCVMode.STCV,'useDirectLighting':False})
    ...
    ToneMapper = createPass("ToneMapper", {'autoExposure': True, 'exposureCompensation': 0.0, 'operator': ToneMapOp.Linear})
...
#scene_path = os.environ.get("RESTIR_PT_DEMO_SCENE", os.path.join(project_root, "Media", "TestScenes", "CornellBox.pyscene"))
scene_path = 'VeachAjar/VeachAjarAnimated.pyscene'
